...
def _extract_type_fields(body_lines):
    ...
    for line in body_lines:
        match = _TYPE_FIELD_RE.match(line)
        if match:
            type_map.setdefault(match.group(1), match.group(2))
            continue
        match = _RTYPE_FIELD_RE.match(line)
...
class StubGenerator(object):
    ...
    def _build_signature(self, name, block, qualified, in_class, is_static):
        ...
        if not all(re.match(r'^\w+$', parameter.name) for parameter in parameters):
            ...
            return_type = 'None' if name == '__init__' else \
                self._parse_type_text(rtype, '%s -> ' % qualified)
...
def main():
    ...
    if generator.unparsed_report:
        print('Unparsed type expressions (emitted as Any) - the docstring-improvement '
              'worklist:', file=sys.stderr)
        for line in sorted(generator.unparsed_report):
