...
# not depend on any GUI/GL library (Qt Widgets, Qt Svg, the Qt OpenGL modules, Qwt, GLEW,
# OpenGL). Registered as the "pygplates-linkage-test" CTest.
#
...
# framework). "opengl" also catches Qt6OpenGL/Qt6OpenGLWidgets, opengl32.dll and
# libOpenGL.so (GLVND).
...
def main():
    ...
    if forbidden:
        sys.exit('\n%d forbidden GUI/GL dependencies - the module must not link these '
                 '(see doc-cpp/design/architecture/README.md)' % len(forbidden))
    print('OK: no GUI/GL dependencies')
    return 0
