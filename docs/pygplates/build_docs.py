...
pygplates module:
  ...
  * a CMake build tree  - pass '--module-dir' (used by the 'docs-pygplates' CMake target, and by
...
The steps mirror 'docs/pygplates/CMakeLists.txt': copy the '.rst' sources, images and '_static'
...
def populate_scratch_dir(scratch_dir):
    """Copy the '.rst' sources, sample scripts, images and '_static' into the Sphinx source dir.
    ...
    # The sample scripts ('sample-code/*.py') go with the pages: they include the scripts' code
    # through the 'sample-code' directive (see 'conf.py.in'), relative to the page.
    for subdir, pattern in (("sample-code", "*.rst"), ("sample-code", "*.py"), ("images", "*.png"),
                            ("_static", "*")):
