...
std::auto_ptr<GPlatesOpenGL::GLVertexElementBuffer>
GPlatesOpenGL::GLVertexElementBuffer::create_as_auto_ptr(
		GLRenderer &renderer,
		const GLBuffer::shared_ptr_type &buffer)
{
	...
	if (buffer_object)
	{
		return std::auto_ptr<GPlatesOpenGL::GLVertexElementBuffer>(
				GLVertexElementBufferObject::create_as_auto_ptr(renderer, buffer_object));
	}
	...
	return std::auto_ptr<GPlatesOpenGL::GLVertexElementBuffer>(
			GLVertexElementBufferImpl::create_as_auto_ptr(renderer, buffer_impl));
}
