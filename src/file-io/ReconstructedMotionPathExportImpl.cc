...
namespace
{
	...
	typedef GPlatesFileIO::ReconstructedMotionPathExportImpl::referenced_files_collection_type
			referenced_files_collection_type;

	//! Convenience typedef for active reconstructable files.
	typedef GPlatesFileIO::ReconstructedMotionPathExportImpl::files_collection_type
			files_collection_type;

	//! Convenience typedef for sequence of motion_paths.
	...
	typedef GPlatesFileIO::ReconstructedMotionPathExportImpl::feature_handle_to_collection_map_type
			feature_handle_to_collection_map_type;

	//! Convenience typedef for a sequence of grouped RFGs.
	typedef GPlatesFileIO::ReconstructedMotionPathExportImpl::motion_path_group_seq_type
			feature_geometry_group_seq_type;

	class ContainsSameFilePointerPredicate: public std::unary_function<
		GPlatesFileIO::ReconstructedMotionPathExportImpl::FeatureCollectionMotionPathGroup,bool>
	{
	public:
		bool 
		operator()(
			const GPlatesFileIO::ReconstructedMotionPathExportImpl::FeatureCollectionMotionPathGroup& elem) const
		{
			...
			const feature_handle_to_collection_map_type &feature_handle_to_collection_map)
	{
		...
		for (rmt_iter = reconstructed_motion_path_seq.begin();
			rmt_iter != reconstructed_motion_path_seq.end();
			++rmt_iter)
		{
...
void
GPlatesFileIO::ReconstructedMotionPathExportImpl::get_files_referenced_by_geometries(
		referenced_files_collection_type &referenced_files,
		const reconstructed_motion_path_seq_type &reconstructed_motion_path_seq,
		const files_collection_type &reconstructable_files,
		feature_handle_to_collection_map_type &feature_handle_to_collection_map)
...
void
GPlatesFileIO::ReconstructedMotionPathExportImpl::group_motion_paths_with_their_feature(
		motion_path_group_seq_type &grouped_motion_paths_seq,
		const reconstructed_motion_path_seq_type &reconstructed_motion_path_seq)
{
	// Copy sequence so we can sort the motion_paths by feature.
	reconstructed_motion_path_seq_type motion_paths_sorted_by_feature(
			reconstructed_motion_path_seq);
	...
	std::sort(motion_paths_sorted_by_feature.begin(), motion_paths_sorted_by_feature.end(),
		boost::lambda::bind(&GPlatesAppLogic::ReconstructedMotionPath::feature_handle_ptr, _1) <
					boost::lambda::bind(
							&GPlatesAppLogic::ReconstructedMotionPath::feature_handle_ptr, _2));
	...
	// into a group.
	reconstructed_motion_path_seq_type::const_iterator sorted_motion_paths_iter;
	for (sorted_motion_paths_iter = motion_paths_sorted_by_feature.begin();
		sorted_motion_paths_iter != motion_paths_sorted_by_feature.end();
		++sorted_motion_paths_iter)
	{
		const GPlatesAppLogic::ReconstructedMotionPath *rmt = *sorted_motion_paths_iter;
		const GPlatesModel::FeatureHandle *feature_handle_ptr = rmt->feature_handle_ptr();
		...
		if (feature_handle_ptr != current_feature_handle_ptr)
		{
			...
			grouped_motion_paths_seq.push_back(MotionPathGroup(feature_ref));

			current_feature_handle_ptr = feature_handle_ptr;
		}

		// Add the current reconstructed motion_path to the current feature.
		grouped_motion_paths_seq.back().recon_motion_paths.push_back(rmt);
	}
}

void
GPlatesFileIO::ReconstructedMotionPathExportImpl::group_motion_path_groups_with_their_collection(
	const feature_handle_to_collection_map_type &feature_handle_to_collection_map,
	...
	const motion_path_group_seq_type &grouped_motion_paths_seq)
{
	...
	feature_geometry_group_seq_type::const_iterator feature_iter = grouped_motion_paths_seq.begin();
	for (; feature_iter != grouped_motion_paths_seq.end() ; ++feature_iter)
	{
		...
		if (map_iter != feature_handle_to_collection_map.end())
		{
			...
			feature_collection_motion_path_group_seq_type::iterator it =
			...
			if (it != grouped_features_seq.end())
			{
				...
				it->motion_path_groups.push_back(*feature_iter);
			}
			else
			{
				// We have found a new collection, so create an entry in the feature_collection_feature_group_seq
				FeatureCollectionMotionPathGroup group_of_features(file_ptr);
				group_of_features.motion_path_groups.push_back(*feature_iter);
