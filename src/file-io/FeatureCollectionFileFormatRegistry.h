...
namespace GPlatesFileIO
{
	...
	{
		/**
		 * Base class for specifying options for reading and/or writing a feature collection from/to a file.
		 *
		 * If a file format requires specialised options then create a derived
		 * @a ReadWriteOptions class for it and register that with @a Registry.
		 */
		class ReadWriteOptions :
				public GPlatesUtils::ReferenceCount<ReadWriteOptions>
		{
		public:
			typedef GPlatesUtils::non_null_intrusive_ptr<ReadWriteOptions> non_null_ptr_type;
			typedef GPlatesUtils::non_null_intrusive_ptr<const ReadWriteOptions> non_null_ptr_to_const_type;

			virtual
			~ReadWriteOptions()
			{  }

			//! Clones derived read/write options object.
			virtual
			non_null_ptr_type
			clone() const = 0;
		};
		...
		public:
			...
			typedef boost::function<
					void (
							...
							const boost::optional<ReadWriteOptions::non_null_ptr_to_const_type> &)>
			 ...
			 * @param default_write_options the default write options to use when writing a feature collection.
			 *        Is boost::none if file format does not have write options.
			 */
			void
			...
			/**
			 * Reads features from a file (of format @a file_format) into the file's feature collection.
			 *
			 * Note that the filename's extension is *not* used to determine the file format.
			 * This should typically be used with @a get_read_file_format to determine the file
			 * format based on the filename (and/or file contents).
			 *
			 ...
			 * @param read_options options used when reading from the file - if none are provided
			 *        then the default read/write options registered for the file format are used.
			 *
			...
			read_feature_collection(
					...
					const boost::optional<ReadWriteOptions::non_null_ptr_to_const_type> &read_options = boost::none) const;
			 ...
			 * The file format is determined by the filename (and maybe even the file contents)
			 * provided by @a file_ref.
			 *
			 * @param file_ref both filename of file to read from and feature collection to store in.
			 * @param read_errors to contain errors reading file.
			 * @param read_options options used when reading from the file - if none are provided
			 *        then the default read/write options registered for the file format are used.
			 *
			 * @throws FileFormatNotSupportedException if file format is not recognised, or if
			 * file format does not support reading (eg, only writing) or file format is not registered.
			 */
			void
			read_feature_collection(
					const File::Reference &file_ref,
			 ...
			 *
			 * Note that the filename's extension is *not* used to determine the file format.
			 * This should typically be used with @a get_write_file_format to determine the file
			 * format based on the filename.
			 *
			 * @param file_format the file format with which to write to the file.
			 ...
			 * @param write_options options used when writing to the file - if none are provided
			 *        then the default write options registered for the file format are used.
			 *
			...
			write_feature_collection(
					...
					const boost::optional<ReadWriteOptions::non_null_ptr_to_const_type> &write_options = boost::none) const;

			/**
			 * Writes features from the specified feature collection to the specified output file.
			 *
			 * Note that the file format to write with is determined by the filename extension.
			 *
			 * @param file_info output file that feature collection is written to.
			 * @param feature_collection the feature collection that will be written.
			 * @param write_options options used when writing to the file - if none are provided
			 ...
			 * @throws ErrorOpeningFileForWritingException if file is not writable.
			 * @throws FileFormatNotSupportedException if file format is not recognised, or if
			 * file format does not support writing (eg, only reading) or file format is not registered.
			 */
			void
			write_feature_collection(
					const GPlatesModel::FeatureCollectionHandle::const_weak_ref &feature_collection,
			...
			/**
			 * Returns the default read/write options for the specified file format.
			 *
			 * Returns boost::none if specified file format has no read or write options.
			 *
			 * @throws FileFormatNotSupportedException if file format no been registered.
			 */
			const boost::optional<ReadWriteOptions::non_null_ptr_to_const_type> &
			get_default_read_write_options(
					Format file_format) const;

			/**
			 * Sets the default read/write options for the specified file format.
			 *
			 * @throws FileFormatNotSupportedException if file format no been registered.
			 */
			void
			set_default_read_write_options(
					Format file_format,
		...
		private:
			struct FileFormatInfo
			{
				FileFormatInfo(
						...
						const boost::optional<ReadWriteOptions::non_null_ptr_to_const_type> &default_read_write_options_) :
					...
					default_read_write_options(default_read_write_options_)
				{  }
				...
				boost::optional<ReadWriteOptions::non_null_ptr_to_const_type> default_read_write_options;
			};
