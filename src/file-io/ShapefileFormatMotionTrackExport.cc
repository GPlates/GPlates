...
namespace
{
	...
	/**
	 * Fill a kvd with data describing how the motion_tracks were generated. 
	 *
		...
		bool should_add_referenced_files = true)
	{
		...
		XsString::non_null_ptr_type motion_track_seeds_value = 
			XsString::create(GPlatesUtils::make_icu_string_from_qstring(seed_string));

		GpmlKeyValueDictionaryElement motion_track_seeds_element(
			key,
			motion_track_seeds_value,
			TemplateTypeParameterType::create_xsi("string"));
		dictionary->elements().push_back(motion_track_seeds_element);
		...
		if (should_add_referenced_files)
		{
			...
			int file_count = 1;
			GPlatesFileIO::ShapefileFormatMotionTrackExport::referenced_files_collection_type::const_iterator file_iter;
...
void
GPlatesFileIO::ShapefileFormatMotionTrackExport::export_motion_tracks(
		...
		bool should_export_referenced_files)
{
	...
	for (; iter != end ; ++iter)
	{
		...
		ReconstructedMotionTrackExportImpl::reconstructed_motion_track_seq_type::const_iterator rmt_iter;
		for (rmt_iter = motion_track_group.recon_motion_tracks.begin();
			rmt_iter != motion_track_group.recon_motion_tracks.end();
			++rmt_iter)
		{
			...
			exporter.export_geometry(rmt->motion_track_points(),kvd);
