...
#include "app-logic/MotionTrackUtils.h"
...
namespace
{
	...
	typedef GPlatesFileIO::GMTFormatMotionTracksExport::referenced_files_collection_type
	...
	*/
	...
	write_seed_point_to_stream(
		...
		const GPlatesAppLogic::ReconstructedMotionTrack &rf)
	{
		GPlatesAppLogic::ReconstructedMotionTrack::seed_point_geom_ptr_type seed_point =
			rf.seed_point();
	...
	void
	write_motion_track_to_stream(
		QTextStream &text_stream,
		const GPlatesAppLogic::ReconstructedMotionTrack &rmt,
		const std::vector<double> &times)
	{
		GPlatesAppLogic::ReconstructedMotionTrack::motion_track_geom_ptr_type mt = rmt.motion_track_points();
	...
	get_global_header_lines(
		...
		const GPlatesFileIO::GMTFormatMotionTracksExport::referenced_files_collection_type referenced_files,
	...
	// Adapted from GMTFormatReconstructedFeatureGeometryExport.cc
		...
		GPlatesFileIO::GMTFormatMotionTracksExport::referenced_files_collection_type::const_iterator file_iter;
		...
		std::vector<double> &times)
	{
		...
		GPlatesAppLogic::MotionTrackUtils::MotionTrackPropertyFinder finder;
...
void
GPlatesFileIO::GMTFormatMotionTracksExport::export_motion_tracks(
	...
	const double &reconstruction_time)
{
	...
	motion_track_group_seq_type::const_iterator 
		iter = motion_track_group_seq.begin(),
		end = motion_track_group_seq.end();

	for (; iter != end ; ++iter)
	{
		// Get per-feature stuff: feature info, left/right plates, times.
		const ReconstructedMotionTrackExportImpl::MotionTrackGroup &motion_track_group =
			*iter;

		const GPlatesModel::FeatureHandle::const_weak_ref &feature_ref =
			motion_track_group.feature_ref;
		...
		reconstructed_motion_track_seq_type::const_iterator rf_iter;
		for (rf_iter = motion_track_group.recon_motion_tracks.begin();
			 rf_iter != motion_track_group.recon_motion_tracks.end();
			++rf_iter)
		{
			const GPlatesAppLogic::ReconstructedMotionTrack *rf = *rf_iter;

			// Print the seed point
			write_seed_point_to_stream(output_stream,*rf);
			write_motion_track_to_stream(output_stream,*rf,export_times);
		}
