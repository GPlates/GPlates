...
namespace{
	...
	typedef element_type::const_iterator element_const_iterator_type;
	...
	{
		element_type elements = key_value_dictionary->get_elements();
		if (elements.empty())
		{
		...
		GPlatesPropertyValues::GpmlKeyValueDictionary::non_null_ptr_to_const_type key_value_dictionary)
	{
		if ((ogr_layer != NULL) && (ogr_feature != NULL))
		{
			...
			int num_attributes_in_dictionary = key_value_dictionary->get_elements().size();
			...
			element_const_iterator_type 
				iter = key_value_dictionary->get_elements().begin(),
				end = key_value_dictionary->get_elements().end();

			for (int count = 0; (count < num_attributes_in_layer) && (iter != end) ; count++, ++iter)
			{
				QString model_string = GPlatesUtils::make_qstring_from_icu_string(iter->key()->get_value().get());
		...
		const boost::optional<GPlatesPropertyValues::GpmlKeyValueDictionary::non_null_ptr_to_const_type> &key_value_dictionary)
	{
		...
		if (!ogr_layer)
		{
			...
			if (key_value_dictionary && !((*key_value_dictionary)->get_elements().empty()))
			{
