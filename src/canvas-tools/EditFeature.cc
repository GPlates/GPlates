...
void
GPlatesCanvasTools::EditFeature::handle_left_click(
		...
		bool is_on_globe)
{
	...
	// Update the focused feature, and pop up the FeaturePropertiesDialog with the Edit tab open.
	d_feature_focus.set_focused_feature(feature_ref);
	fp_dialog().choose_edit_widget();
	fp_dialog().show();
