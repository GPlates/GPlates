...
export_polyline_on_sphere()
{
	...
	bp::class_<
		...
		.def("rotation_interpolate",
				...
				"the original polylines). Each polyline *from_polyline* and *to_polyline* also have monotonically "
				"decreasing latitudes (in North pole reference frame of *rotation_pole*) and the latitude range of "
				"each polyline is restricted to be between the latitude of its first point and the latitude of its "
				"last point. The polylines *from_polyline* and *to_polyline* are also clipped to have a common "
				"overlapping latitude range (with a certain amount of non-overlapping allowed if "
