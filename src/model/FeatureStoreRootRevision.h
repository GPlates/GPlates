...
namespace GPlatesModel
{
	...
	/**
	 * A feature store root revision contains the revisioned content of a feature store root.
	 *
	 ...
	 *
	 * A FeatureStoreRootRevision instance contains the revisioned content of the feature store
	 * root, and is in turn referenced by either a FeatureStoreRootHandle instance or a
	 * TransactionItem instance.
	 *
	 * A new FeatureStoreRootRevision instance will be created whenever the feature store root
	 * is to be modified by the addition or removal of feature-collection elements -- the
	 * existing ("current") FeatureStoreRootRevision instance will not be modified.  The new
	 * FeatureStoreRootRevision instance will then be "scheduled" in a TransactionItem.  When
	 * the TransactionItem is committed, the reference to the new FeatureStoreRootRevision
	 * instance will be swapped with the reference to the "current" instance, so that the "new"
	 * instance will become the "current" instance (which is referenced by the
	 * FeatureStoreRootHandle) and the "current" instance will become the "old" instance (which
	 * is referenced by the committed TransactionItem).
	 *
	 * Client code should not reference FeatureStoreRootRevision instances directly; rather, it
	 * should always access the "current" instance through the feature store root handle.
	 */
