...
void
GPlatesAppLogic::TopologyBoundaryResolverLayerTask::activate(
		bool active)
{
	// If deactivated then specify an empty set of topological sections layer proxies.
	if (!active)
	{
		d_topology_boundary_resolver_layer_proxy->set_current_topological_sections_layer_proxies(
...
{
	d_topology_boundary_resolver_layer_proxy->set_current_reconstruction_time(reconstruction->get_reconstruction_time());
	...
	d_topology_boundary_resolver_layer_proxy->set_current_topological_sections_layer_proxies(topological_sections_layer_proxies);
