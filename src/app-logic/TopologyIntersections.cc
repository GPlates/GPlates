		...
		const bool previous_section_reverse_hint)
{
	...
	// All sections added to the segment list should have at least one of their
		...
		bool intersect_this_section_head)
{
	...
	// This ensures all sub-segments will be able to return a geometry even
	...
	// If we have processed two intersections on either section then we can set its sub-segment.
	if (++previous_section.d_num_neighbours_intersected == 2)
	{
		previous_section.set_sub_segment();
	}
		...
		TopologicalIntersections &previous_section)
{
	...
	// The middle segments are the sub-segments.
	// These middle segments will always be non-null since there's two intersections
	// which cannot be the same intersection.
	previous_section.d_sub_segment = boost::get<4>(*intersected_segments);
	d_sub_segment = boost::get<7>(*intersected_segments);
...
bool
GPlatesAppLogic::TopologicalIntersections::get_reverse_flag(
...
GPlatesAppLogic::TopologicalIntersections::get_unreversed_sub_segment(
		bool reverse_hint) const
{
	// If we've already determined the sub-segment then return it.
	if (d_sub_segment)
	{
	...
	// the 'd_sub_segment' case above because non-null geometry is always
...
void
GPlatesAppLogic::TopologicalIntersections::set_sub_segment()
{
	...
	if (d_last_intersected_head_segment)
	{
		...
		// This means its sub-segment is the new tail segment from this intersection.
		//
		...
		// This is ok though since it means the sub-segment is zero length
		// and doesn't contribute to the resolved topological geometry.
		...
		// to reverse the sub-segment - otherwise we do need to reverse.
		//
		...
		// order of the sub-segments relative to each other is the same as
		// the order of points along the sub-segments of the current section.
		//
...
void
GPlatesAppLogic::TopologicalIntersections::handle_t_or_v_junction(
