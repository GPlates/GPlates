		...
		const std::vector<ResolvedTopologicalNetwork::non_null_ptr_type> &resolved_topological_networks)
{
	...
	// Associates a sub-segment with its owning resolved topology.
	...
	// Map of each section to all the resolved topologies that have it as a sub-segment.
	typedef std::map<const ReconstructionGeometry *, std::vector<SubSegmentInfo> >
			section_to_sharing_resolved_topologies_map_type;
	section_to_sharing_resolved_topologies_map_type section_to_sharing_resolved_topologies_map;
	...
	BOOST_FOREACH(
			...
			resolved_topological_boundaries)
	{
		const sub_segment_seq_type &sub_segments = resolved_topological_boundary->get_sub_segment_sequence();
		...
		for ( ; sub_segments_iter != sub_segments_end; ++sub_segments_iter)
		{
			...
			section_to_sharing_resolved_topologies_map[sub_segment.get_reconstruction_geometry().get()].push_back(
					SubSegmentInfo(sub_segment, resolved_topological_boundary));
		}
	}
	...
	BOOST_FOREACH(
			...
			resolved_topological_networks)
	{
		const sub_segment_seq_type &sub_segments = resolved_topological_network->get_boundary_sub_segment_sequence();
		...
		for ( ; sub_segments_iter != sub_segments_end; ++sub_segments_iter)
		{
			...
			// Add the current resolved topology to the list of those sharing the current section.
			section_to_sharing_resolved_topologies_map[sub_segment.get_reconstruction_geometry().get()].push_back(
					SubSegmentInfo(sub_segment, resolved_topological_network));
		}
	}
	...
	typedef std::vector<SubSegmentMarker> sub_segment_marker_seq_type;

	// All sub-segment markers on the same polyline segment/arc.
	struct PolylineSegmentMarkers
	{
		sub_segment_marker_seq_type sub_segment_markers;
	};

	typedef std::vector<PolylineSegmentMarkers> polyline_segment_markers_seq_type;
	...
	typedef std::map<unsigned int, unsigned int> polyline_segment_index_to_polyline_segment_markers_map_type;
	...
	// Iterate over the section referenced by resolved topologies.
	BOOST_FOREACH(
			const section_to_sharing_resolved_topologies_map_type::value_type &section_to_sharing_resolved_topologies_entry,
			section_to_sharing_resolved_topologies_map)
	{
		const ReconstructionGeometry *section_rg = section_to_sharing_resolved_topologies_entry.first;
		const std::vector<SubSegmentInfo> &sub_segments = section_to_sharing_resolved_topologies_entry.second;
			...
			section_geometry_type == GPlatesMaths::GeometryType::MULTIPOINT)
		{
			...
			std::vector<SubSegmentInfo>::const_iterator sub_segments_iter = sub_segments.begin();
			std::vector<SubSegmentInfo>::const_iterator sub_segments_end = sub_segments.end();
			for ( ; sub_segments_iter != sub_segments_end; ++sub_segments_iter)
			{
				const SubSegmentInfo &sub_segment_info = *sub_segments_iter;
		...
		polyline_segment_markers_seq_type polyline_segment_markers_seq;

		// Map polyline segment/arc indices into a sequence since not all polyline segments/arcs will
		// contain markers (in fact very few will) - so this just saves a bit of memory and set up time.
		polyline_segment_index_to_polyline_segment_markers_map_type polyline_segment_index_to_polyline_segment_markers_map;
		...
		for ( ; sub_segments_iter != sub_segments_end; ++sub_segments_iter)
		{
			...
			for (n = 0; n < 2; ++n) // n is 'START' and 'END'
			{
				...
				const std::pair<polyline_segment_index_to_polyline_segment_markers_map_type::iterator, bool>
						polyline_segment_index_insert_result =
								polyline_segment_index_to_polyline_segment_markers_map.insert(
										polyline_segment_index_to_polyline_segment_markers_map_type::value_type(
												closest_start_end_segment_indices_in_section_polyline[n],
												polyline_segment_markers_seq.size()/*index*/));
				if (polyline_segment_index_insert_result.second)
				{
					// Insertion successful - first time seen polyline segment.
					polyline_segment_markers_seq.push_back(PolylineSegmentMarkers());
				}
				PolylineSegmentMarkers &polyline_segment_markers = polyline_segment_markers_seq[
				...
				sub_segment_marker_seq_type::iterator sub_segment_markers_iter =
						polyline_segment_markers.sub_segment_markers.begin();
				const sub_segment_marker_seq_type::iterator sub_segment_markers_end =
				...
				{
					const SubSegmentMarker &sub_segment_marker = *sub_segment_markers_iter;
		...
		boost::optional<const SubSegmentMarker &> prev_sub_segment_marker;
		...
		polyline_segment_index_to_polyline_segment_markers_map_type::const_iterator polyline_segment_index_entries_iter =
				polyline_segment_index_to_polyline_segment_markers_map.begin();
		const polyline_segment_index_to_polyline_segment_markers_map_type::const_iterator polyline_segment_index_entries_end =
				polyline_segment_index_to_polyline_segment_markers_map.end();
		for ( ; polyline_segment_index_entries_iter != polyline_segment_index_entries_end; ++polyline_segment_index_entries_iter)
		{
			const polyline_segment_index_to_polyline_segment_markers_map_type::value_type &
					polyline_segment_index_entry = *polyline_segment_index_entries_iter;
			const PolylineSegmentMarkers &polyline_segment_markers =
					polyline_segment_markers_seq[polyline_segment_index_entry.second];

			// Iterate over the sub-segment markers in the current polyline segment/arc.
			sub_segment_marker_seq_type::const_iterator sub_segment_markers_iter =
					polyline_segment_markers.sub_segment_markers.begin();
			const sub_segment_marker_seq_type::const_iterator sub_segment_markers_end =
					polyline_segment_markers.sub_segment_markers.end();
			for ( ;
