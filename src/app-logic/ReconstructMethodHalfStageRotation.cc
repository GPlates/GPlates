	...
	namespace
	{
		...
		protected:
					...
					GPlatesModel::FeatureHandle &feature_handle)
			{
				...
				// If we can't get left/right plate IDs then we'll just reconstruct using the
				// identity rotation - we may need to make this an option - some users might not want to
				// see a reconstructed geometry if a feature has no left/right plate IDs.
					...
					d_reconstruction_params.get_right_plate_id())
				{
					const GPlatesModel::integer_plate_id_type left_plate_id =
							d_reconstruction_params.get_left_plate_id().get();
...
void
GPlatesAppLogic::ReconstructMethodHalfStageRotation::get_present_day_geometries(
...
void
GPlatesAppLogic::ReconstructMethodHalfStageRotation::reconstruct_feature(
		std::vector<ReconstructedFeatureGeometry::non_null_ptr_type> &reconstructed_feature_geometries,
		...
		const double &reconstruction_time)
{
	...
	ReconstructFeature visitor(reconstructed_feature_geometries, reconstruct_handle, reconstruction_tree);
		...
		bool reverse_reconstruct)
{
	...
	// If we have left/right plate IDs then we can reconstruct.
	if (reconstruction_feature_properties.get_left_plate_id() &&
		reconstruction_feature_properties.get_right_plate_id())
	{
		...
		return ReconstructUtils::reconstruct_as_half_stage(
				geometry,
				reconstruction_feature_properties.get_left_plate_id().get(),
				reconstruction_feature_properties.get_right_plate_id().get(),
				*reconstruction_tree,
				reverse_reconstruct);
	}
