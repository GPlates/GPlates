...
#include "model/TemplateTypeParameterType.h"
...
namespace
{
	...
	geo_time_instants_are_approx_equal(
			const GPlatesModel::GeoTimeInstant &t1,
			const GPlatesModel::GeoTimeInstant &t2)
	{
	...
	struct TotalReconSeqProperties
	{
		...
		boost::intrusive_ptr<GPlatesModel::GpmlIrregularSampling> d_irregular_sampling;
		GPlatesModel::GpmlPlateId::integer_plate_id_type d_fixed_plate_id;
		GPlatesModel::GpmlPlateId::integer_plate_id_type d_moving_plate_id;
	};
			...
			GPlatesFileIO::ReadErrorAccumulation &read_errors)
	{
		...
		while (line_buffer.getline(line_of_input)) {
			...
			GPlatesModel::GpmlPlateId::integer_plate_id_type fixed_plate_id, moving_plate_id;
