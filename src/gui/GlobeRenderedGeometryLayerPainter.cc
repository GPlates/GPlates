		...
		LayerPainter &layer_painter)
{
	...
	const cache_handle_type layer_cache =
			layer_painter.end_painting(gl, view_projection, d_scale, d_globe_horizon_plane);
		...
		const GPlatesViewOperations::RenderedColouredMultiPointOnSphere &rendered_coloured_multi_point_on_sphere)
{
	...
	{
		vertex_colours.push_back(get_vector_geometry_colour(point_colours[c]));
	}
		...
		const GPlatesViewOperations::RenderedString &rendered_string)
{
				...
				GPlatesOpenGL::GLIntersect::Plane::POSITIVE)
	{
		...
		boost::optional<Colour> shadow_colour = rendered_string.get_shadow_colour();
		if (shadow_colour)
		{
			shadow_colour = get_vector_geometry_colour(shadow_colour.get());
		}
