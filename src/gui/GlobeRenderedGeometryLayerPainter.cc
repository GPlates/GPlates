...
namespace 
{
	...
	/**
	 * We will tessellate a small circle arc if the two endpoints are far enough apart.
	 */
	const double SMALL_CIRCLE_ARC_ANGULAR_THRESHOLD = GPlatesMaths::convert_deg_to_rad(1);
	const double COSINE_SMALL_CIRCLE_ARC_ANGULAR_THRESHOLD = std::cos(SMALL_CIRCLE_ARC_ANGULAR_THRESHOLD);
		...
		LayerPainter &layer_painter)
{
	...
	// NOTE: Some rendered geometries will already have been rendered such as rasters.
		...
		const GPlatesViewOperations::RenderedString &rendered_string)
{
	...
	{
		d_layer_painter->text_off_the_sphere.push_back(
				LayerPainter::TextDrawable(
		...
		const GPlatesViewOperations::RenderedSmallCircle &rendered_small_circle)
{
	...
	std::vector<GPlatesMaths::PointOnSphere> points;
	tessellate(points, rendered_small_circle.get_small_circle(), SMALL_CIRCLE_ARC_ANGULAR_THRESHOLD);
		...
		const GPlatesViewOperations::RenderedSmallCircleArc &rendered_small_circle_arc)
{
	...
	std::vector<GPlatesMaths::PointOnSphere> points;
	tessellate(points, rendered_small_circle_arc.get_small_circle_arc(), SMALL_CIRCLE_ARC_ANGULAR_THRESHOLD);
	...
	const GPlatesViewOperations::RenderedCircleSymbol &rendered_circle_symbol)
{
    ...
    else
    {
		...
		std::vector<GPlatesMaths::PointOnSphere> points;
		tessellate(points, small_circle, SMALL_CIRCLE_ARC_ANGULAR_THRESHOLD);
