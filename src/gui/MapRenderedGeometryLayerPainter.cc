...
{
	const Colour &colour = rendered_multi_point_on_sphere.get_colour();
			...
			const GPlatesViewOperations::RenderedColouredMultiPointOnSphere &rendered_coloured_multi_point_on_sphere)	
{
	...
	{
		vertex_colours.push_back(point_colours[c]);
	}
...
{
	const Colour &colour = rendered_point_on_sphere.get_colour();
...
{
	const Colour &colour = rendered_polygon_on_sphere.get_colour();
...
{
	const Colour &colour = rendered_polyline_on_sphere.get_colour();
	...
	const GPlatesViewOperations::RenderedColouredEdgeSurfaceMesh &rendered_coloured_edge_surface_mesh)
{
	...
	if (rendered_coloured_edge_surface_mesh.get_use_vertex_colours())
	{
		...
		{
			vertex_colours.push_back(mesh_colours[c]);
		}
	...
	else // edge colouring ...
	{
		...
		for (unsigned int e = 0; e < num_mesh_edges; ++e)
		{
			...
			const Colour &edge_colour = mesh_colours[e];

			// Convert colour from floats to bytes to use less vertex memory.
			const rgba8_t edge_rgba8_color = Colour::to_rgba8(edge_colour);
		...
		const GPlatesViewOperations::RenderedColouredTriangleSurfaceMesh &rendered_coloured_triangle_surface_mesh)
{
	...
	if (rendered_coloured_triangle_surface_mesh.get_use_vertex_colours())
	{
		...
		for (unsigned int c = 0; c < num_mesh_colours; ++c)
		{
			...
			// Modulate with the fill modulate colour...
			Colour vertex_colour = Colour::modulate(
					mesh_colours[c],
...
{
	const Colour &colour = rendered_small_circle.get_colour();
...
{
	const Colour &colour = rendered_small_circle_arc.get_colour();
...
{
	const Colour &colour = rendered_ellipse.get_colour();
	...
	const GPlatesViewOperations::RenderedArrow &rendered_arrow)
{
	...
	const GPlatesMaths::PointOnSphere arrow_start_globe = rendered_arrow.get_start_position();
	const GPlatesMaths::LatLonPoint arrow_start_llp = make_lat_lon_point(arrow_start_globe);
	const GPlatesMaths::Vector3D arrow_start_vec(arrow_start_globe.position_vector());
	...
	GPlatesMaths::CartesianConvMatrix3D ccm(arrow_start_globe);
	const GPlatesMaths::Vector3D arrow_vector_ned = convert_from_geocentric_to_north_east_down(
			ccm,
	...
	const GPlatesMaths::Vector3D arrow_vector(arrow_vector_map_x, arrow_vector_map_y, arrow_vector_map_z);

	const QPointF arrow_start_map = get_projected_unwrapped_position(arrow_start_globe);
	const GPlatesMaths::Vector3D arrow_start(arrow_start_map.x(), arrow_start_map.y(), 0.0);
	...
	const GPlatesMaths::Vector3D arrow_end = arrow_start + GLOBE_TO_MAP_SCALE_FACTOR * d_inverse_zoom_factor * arrow_vector;
	...
	GPlatesMaths::real_t arrow_body_width = GLOBE_TO_MAP_SCALE_FACTOR * d_inverse_zoom_factor * rendered_arrow.get_arrow_body_width();
	GPlatesMaths::real_t arrowhead_size = GLOBE_TO_MAP_SCALE_FACTOR * d_inverse_zoom_factor * rendered_arrow.get_arrowhead_size();
...
{
	const Colour &colour = rendered_arrowed_polyline.get_colour();
...
{
	const Colour &colour = rendered_triangle_symbol.get_colour();
...
{
	const Colour &colour = rendered_square_symbol.get_colour();
...
{
	const Colour &colour = rendered_circle_symbol.get_colour();
	...
	const GPlatesViewOperations::RenderedCrossSymbol &rendered_cross_symbol)
{
	...
	const Colour &colour = rendered_cross_symbol.get_colour();
...
void
GPlatesGui::MapRenderedGeometryLayerPainter::paint_arrow(
...
void
GPlatesGui::MapRenderedGeometryLayerPainter::paint_arrow_head_3D(
...
void
GPlatesGui::MapRenderedGeometryLayerPainter::paint_arrow_head_2D(
