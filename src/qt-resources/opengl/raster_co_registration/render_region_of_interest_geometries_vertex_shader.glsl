...
#ifdef POINT_REGION_OF_INTEREST
	in vec3 point_centre;
	// The 'xyz' values are (weight_tangent_x, weight_tangent_y, weight_point_centre)
	in vec3 tangent_frame_weights;
#endif

#ifdef LINE_REGION_OF_INTEREST
	in vec3 line_arc_start_point;
	in vec3 line_arc_normal;
	// The 'xy' values are (weight_arc_normal, weight_start_point)
	in vec2 tangent_frame_weights;
#endif

#ifdef FILL_REGION_OF_INTEREST
	in vec3 fill_position;
#endif

in vec4 world_space_quaternion;

#ifdef ENABLE_SEED_FRUSTUM_CLIPPING
	// The 'xyz' values are (translate_x, translate_y, scale)
	in vec3 raster_frustum_to_seed_frustum_clip_space_transform;
	// The 'xyz' values are (translate_x, translate_y, scale)
	in vec3 seed_frustum_to_render_target_clip_space_transform;
#endif
...
void main (void)
{
	...
	vs_out.present_day_point_centre = point_centre;
   ...
   // Transform present-day vertex position using finite rotation quaternion.
	...
	vec3 rotated_position = rotate_vector_by_quaternion(world_space_quaternion, vs_out.present_day_position);
#endif

#ifdef LINE_REGION_OF_INTEREST
	// Pass the present-day line arc normal to the fragment shader.
	vs_out.present_day_line_arc_normal = line_arc_normal;
   ...
   // Transform present-day position using finite rotation quaternion.
	...
	vec4 raster_frustum_position = view_projection * vec4(rotated_position, 1);
	...
	vs_out.clip_position_params = vec4(
